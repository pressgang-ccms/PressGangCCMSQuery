...
public class TranslatedTopicDataFilterQueryBuilder extends BaseTopicFilterQueryBuilder<TranslatedTopicData> {
    ...
    public TranslatedTopicDataFilterQueryBuilder(final EntityManager entityManager) {
        super(TranslatedTopicData.class, new TranslatedTopicFieldFilter(), entityManager, true);
    ...
    public Predicate getMatchingLocaleString(final String locale) {
        ...
        final CriteriaBuilder criteriaBuilder = getCriteriaBuilder();
        final Predicate localePredicate = criteriaBuilder.equal(getOriginalRootPath().get("locale").get("value"), locale);

        if (defaultLocale != null && defaultLocale.toLowerCase().equals(locale.toLowerCase()))
            return criteriaBuilder.or(localePredicate, criteriaBuilder.isNull(getOriginalRootPath().get("locale").get("value")));
    ...
    public Predicate getNotMatchingLocaleString(final String locale) {
        ...
        final CriteriaBuilder criteriaBuilder = getCriteriaBuilder();
        final Predicate localePredicate = criteriaBuilder.notEqual(getOriginalRootPath().get("locale").get("value"), locale);

        if (defaultLocale != null && defaultLocale.toLowerCase().equals(locale.toLowerCase()))
            return criteriaBuilder.and(localePredicate, criteriaBuilder.isNotNull(getOriginalRootPath().get("locale").get("value")));
    ...
    public void processField(final FilterFieldDataBase<?> field) {
        ...
        if (fieldName.equals(CommonFilterConstants.TOPIC_LATEST_TRANSLATIONS_FILTER_VAR)) {
            ...
            if (conditions.size() > 1) {
                ...
                fieldConditions.add(getCriteriaBuilder().or(predicates));
